...
int performAdventurerCardEffect(int currentPlayer, struct gameState *state)
{
  ...
  while(drawntreasureCount < 2) {
    ...
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      ...
      temphand[temphandSize] = cardDrawn;
...
int performVillageCardEffect(int currentPlayer, int handPos, struct gameState *state)
{
  //+1 Card
  drawCard(currentPlayer, state);
...
int performTributeCardEffect(int currentPlayer, int handPos, struct gameState *state)
{
  int nextPlayer = (currentPlayer + 1) % MAX_PLAYERS;
...
int performFeastCardEffect(int currentPlayer, int choice, int handPos, struct gameState *state)
{
  int temphand[MAX_HAND];
  int i;
  int x;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case feast:
      return performFeastCardEffect(currentPlayer, choice1, handPos, state);
    case gardens:
      return -1;
    ...
    case village:
      return performVillageCardEffect(currentPlayer, handPos, state);
    ...
    case tribute:
      return performTributeCardEffect(currentPlayer, handPos, state);
    case ambassador:
